...
func generateCertificateIdentity() {
	u, err := user.Current()
	if err == nil {
		certificateIdentity = u.Username + "@"
	}
	if h, err := os.Hostname(); err == nil {
		certificateIdentity += h
	}
	if err == nil && u.Name != "" && u.Name != u.Username {
		certificateIdentity += " (" + u.Name + ")"
	}
}

func (m *simbacert) makeCert(hosts []string) {
	if m.caKey == nil {
	...
	var organization = "development certificate"
	var organizationalUnit = certificateIdentity
	if m.config.Organization != "" {
		organization = m.config.Organization
	}
	if m.config.OrganizationalUnit != "" {
		organizationalUnit = m.config.OrganizationalUnit
	}
...
func (m *simbacert) printHosts(hosts []string) {
...
func (m *simbacert) generateKey(rootCA bool) (crypto.PrivateKey, error) {
	if m.ecdsa {
...
func (m *simbacert) fileNames(hosts []string) (certFile, keyFile, p12File string) {
...
func (m *simbacert) makeCertFromCSR() {
	if m.caKey == nil {
		log.Fatalln("ERROR: can't create new certificates because the CA key (rootCA-key.pem) is missing")
	}
...
func (m *simbacert) loadCA() {
	if m.config.Ca.CertName != "" {
		rootName = m.config.Ca.CertName
	}
	if m.config.Ca.KeyName != "" {
		rootKeyName = m.config.Ca.KeyName
	}
...
func (m *simbacert) newCA() {
	...
	tpl := &x509.Certificate{
		...
		Subject: pkix.Name{
			...
			// https://github.com/FiloSottile/simbacert/issues/47
			CommonName: organizationalUnit + " Certificate",
		},
		SubjectKeyId: skid[:],
...
func (m *simbacert) caUniqueName() string {
	var organization = "development certificate"
	if m.config.Organization != "" {
		organization = m.config.Organization
	}
	return organization + " CA " + m.caCert.SerialNumber.String()
}
