...
func (m *simbacert) checkNSS() bool {
	if !hasCertutil {
		return false
	}
	success := true
...
func (m *simbacert) installNSS() bool {
	if m.forEachNSSProfile(func(profile string) {
	...
	if !m.checkNSS() {
		log.Printf("Installing in %s failed. Please report the issue with details about your environment at https://github.com/FiloSottile/simbacert/issues/new 👎", NSSBrowsers)
...
func (m *simbacert) forEachNSSProfile(f func(profile string)) (found int) {
